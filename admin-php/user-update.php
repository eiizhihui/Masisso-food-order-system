...
if ($role === 'Customer') {
    ...
    if(isset($data['bowls'])) $sql .= "points='" . (int)$data['bowls'] . "', ";
    if(isset($data['password']) && !empty($data['password'])) $sql .= "password='" . mysqli_real_escape_string($conn, $data['password']) . "', ";
    ...
    if(isset($data['email'])) $sql .= "email='" . mysqli_real_escape_string($conn, $data['email']) . "', ";
    if(isset($data['phone'])) $sql .= "phone='" . mysqli_real_escape_string($conn, $data['phone']) . "', ";
    if(isset($data['gender'])) $sql .= "gender='" . mysqli_real_escape_string($conn, $data['gender']) . "', ";
    if(isset($data['branch'])) $sql .= "branch='" . mysqli_real_escape_string($conn, $data['branch']) . "', ";
    if(isset($data['role'])) $sql .= "position='" . mysqli_real_escape_string($conn, $data['role']) . "', ";
    if(isset($data['password']) && !empty($data['password'])) $sql .= "password='" . mysqli_real_escape_string($conn, $data['password']) . "', ";
